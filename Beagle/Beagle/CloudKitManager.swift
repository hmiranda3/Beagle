...
//  Beagle
//
//  Created by Habib Miranda on 7/18/16.
...
class CloudKitManager {
    ...
    // So user can recieve notifications when specific things change.
    func subscribe(type: String, predicate: NSPredicate, subscriptionID: String, contentAvailable: Bool, alertBody: String? = nil, desiredKeys: [String]? = nil, options: CKSubscriptionOptions, completion: ((subscription: CKSubscription?, error: NSError?) -> Void)?) {
        ...
        //CKNotificationInfo help outline what info will need to be pushed to us as a notification.
    ...
    //We take the subscription that we created in the public database above and delete it.
    ...
    //Here we are fetching all subscriptions available. This tells cloud kit to bring down all of them.
    ...
    //Here we are working with one in particular, so we need to pass it in the parameter.
